...
int client_sync_file(const char *path, const csiebox_client *client)
{
	...
	if(fcntl(fd, F_SETLK, &setfl) < 0){
		fprintf(stderr, "set read lock fail\n");
	}
	csiebox_protocol_status status = client_send_meta( path,client);
	//struct flock unfl;
	//unfl.l_type = F_UNLCK;
	...
	//	fprintf(stderr, "set unlock fail\n");
	//}
	if( status == CSIEBOX_PROTOCOL_STATUS_OK )
	{
		return 0;
...
int client_send_meta( const char *path, const csiebox_client *client )
{
	...
	int busy;
	while(1){
		recv_message(client->conn_fd, &busy, sizeof(int));
		if(busy == 1){
			printf("Server busy\n");
			fflush(stdout);
			sleep(3);
...
int client_send_file( const char *path, const csiebox_client *client)
{
	...
	while(1){
		...
		if(block == 1){
			...
			sleep(3);
			int wait = 1;
			send_message(client->conn_fd, &wait, sizeof(int));
		}
		else{
			break;
		}
	...
	do
	{
		fprintf(stderr, "block 3 here!!\n");
		nByte_read = read(file_fd,buf,4096);
		fprintf(stderr, "block 4 here!!\n");
		// fill file req
...
int client_rm( const char *path, const csiebox_client *client)
{
	...
	int block;
	while(1){
		recv_message(client->conn_fd, &block, sizeof(int));
		if(block == 1){
			printf("Server block\n");
			fflush(stdout);
			sleep(3);
