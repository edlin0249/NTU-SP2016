...
int server_sync_meta( const csiebox_protocol_meta req, int conn_fd, const csiebox_server *server )
{
	...
	if( lstat(path,&stat) < 0 )
	{
		...
		else if( S_ISREG(req.message.body.stat.st_mode) )
		{
			...
			struct flock getfl;
			getfl.l_type = F_WRLCK;
			getfl.l_whence = SEEK_SET;
			getfl.l_start = 0;
			getfl.l_len = 0;
			getfl.l_pid = getpid();
			if(fcntl(fd, F_GETLK, &getfl) < 0){
				fprintf(stderr, "F_GETLK fail\n");
			}
			int block;
			while(getfl.l_type != F_UNLCK){
				block = 1;
				int wait;
				send_message(conn_fd, &block, sizeof(int));
				recv_message(conn_fd, &wait, sizeof(int));
				getfl.l_type = F_WRLCK;
				getfl.l_whence = SEEK_SET;
				getfl.l_start = 0;
				getfl.l_len = 0;
				getfl.l_pid = getpid();
				fcntl(fd, F_GETLK, &getfl);
			}
			block = 0;
			send_message(conn_fd, &block, sizeof(int));
			struct flock setfl;
			setfl.l_type = F_WRLCK;
			setfl.l_whence = SEEK_SET;
			setfl.l_start = 0;
			setfl.l_len = 0;
			setfl.l_pid = getpid();
			if(fcntl(fd, F_SETLK, &setfl) >= 0){
				fprintf(stderr, "add write lock(exclusive lock) on the file\n");
			}
			...
			struct flock unfl;
			unfl.l_type = F_UNLCK;
			unfl.l_whence = SEEK_SET;
			unfl.l_start = 0;
			unfl.l_len = 0;
			unfl.l_pid = getpid();
			if(fcntl(fd, F_SETLK, &unfl) >= 0){
				fprintf(stderr, "unlock on the file\n");
			}
			close(fd);
		}
...
int server_sync_file( int conn_fd, const char *path )
{
	...
	do
	{
		...
		fprintf(stderr, "will recv_message\n");
		...
		if (file.message.header.req.magic != CSIEBOX_PROTOCOL_MAGIC_REQ) {
			fprintf(stderr, "file.message.header.req.magic : %04x\n",file.message.header.req.magic );
		...
		fprintf(stderr, "block 2 here\n");
		if(!strcmp(path, "../sdir/eddielin//test.txt")){
...
int server_rm( const csiebox_protocol_rm req, int conn_fd, const csiebox_server *server)
{
	...
	if(fcntl(fd, F_GETLK, &getfl) < 0){
		fprintf(stderr, "get unlock fail\n");
	}
	int block;
	while(getfl.l_type != F_UNLCK){
		block = 1;
