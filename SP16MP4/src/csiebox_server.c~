...
void* hande_requests_loop(void* data);
static void serverbusy(csiebox_server* server, int conn_fd);
...
void* handle_requests_loop(void* data) {
  ...
  fprintf(stderr, "thread '%d' after pthread_mutex_lock\n", thread_id);
  fflush(stdout);
  thread_count++;
  /* do forever.... */
  while (1) {
	...
	fdcond[a_request->fd] = 1;
        handle_request(server, a_request->fd);
	//fprintf(stderr, "before thread_count = %d\n", thread_count);
	fdcond[a_request->fd] = 0;
        thread_count++;
	//fprintf(stderr, "after thread_count = %d\n", thread_count);
        free(a_request);
      }
    }
    else {
      ...
      /* requests list.                                       */
      //fprintf("thread '%d' before pthread_cond_wait\n", thread_id);
      //fflush(stdout);
      rc = pthread_cond_wait(&got_request, &request_mutex);
      /* and after we return from pthread_cond_wait, the mutex  */
      /* is locked again, so we don't need to lock it ourselves */
      //fprintf("thread '%d' after pthread_cond_wait\n", thread_id);
      //fflush(stdout);
    }
  }
}
...
//serverbusy == current no available threads in thread pools
static void serverbusy(csiebox_server* server, int conn_fd){
  ...
  fprintf(stderr, "serverbusy is over\n");
}
...
int csiebox_server_run(csiebox_server* server) {
  int i;
  //int thr_id[(*server)->arg.thread_num];
  ...
  while (1) {
	...
	if( FD_ISSET(server->listen_fd, &read_set))
	{
	    ...
	    if(thread_count > 0){
                thread_count--;
          	add_request(conn_fd, &request_mutex, &got_request);
            }
            else{
          	printf("Server busy\n");
		fflush(stdout);
          	serverbusy(server, conn_fd);
            }
		...
		for( i = 0; i < getdtablesize(); ++i )
		{
			if( !server->client[i])
				continue;
			if( FD_ISSET(server->client[i]->conn_fd, &read_set) && fdcond[server->client[i]->conn_fd] == 0)
			{
				//=================================================
				//						TODO
        ...
        if(thread_count > 0){
          thread_count--;
          //fdcondition[i] = 1;
          add_request(server->client[i]->conn_fd, &request_mutex, &got_request);
        }
        else{
          printf("Server busy\n");
	  fflush(stdout);
          serverbusy(server, server->client[i]->conn_fd);
        }
